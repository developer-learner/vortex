# ERD-DELTA v33 — LM Studio library discovery (browse tier), scope fix

This re-freeze carries the v32 discovery feature UNCHANGED and fixes the run
scope only. The v29 orchestrate baseline drags `src/vortex/memory.py` (a v31
direct-route RAM-parity change, already implemented and green) into the delta
inventory, which forced EM full-emission and re-editing of finished work. This
freeze declares `src/vortex/memory.py` as **no_edit** (acceptance-only carry):
its frozen tests still run, but the coder is never invoked for it, so the
mechanical-synthesis lane can drive the three real files from the verbatim
briefs below. No behavioral change, no new AC, no test change.
